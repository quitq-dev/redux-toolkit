...
interface PostItemType {
  post: Post
  handleDeleteItem: (postId: string) => void
  handleStartEditPost: (postId: string) => void
}

export const PostItem = ({ post, handleDeleteItem, handleStartEditPost }: PostItemType) => {
  return (
    <div>
      <div className='flex flex-col items-center overflow-hidden rounded-lg border md:flex-row'>
        <div className='group relative block h-48 w-full shrink-0 self-start overflow-hidden bg-gray-100 md:h-[335px] md:w-32 lg:w-48'>
          <img
        ...
        <div className='flex flex-col gap-2 p-4 lg:p-6'>
          ...
          <h2 className='line-clamp text-xl font-bold text-gray-800'>{post.title}</h2>
